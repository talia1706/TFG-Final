...
@section('titulo', 'Añadir Animales')
@section('contenido')
<?php
...
<div class="mx-auto max-w-lg">
    <h1 class="text-center text-2xl font-bold text-green-600 sm:text-3xl mb-6">Añadir Animales</h1>
    <form action="{{route('superusuario.insertar_animal')}}" id="form" novalidate enctype="multipart/form-data" method="POST">
        @csrf
        <div class="relative z-0 w-full mt-2">
            ...
            />
            <label for="Nombre" class="absolute duration-300 top-3 -z-1 origin-0 text-gray-500">Introduce el nombre del animal</label>
        </div>
        @error('nombre')
    ...
    @enderror
        ...
        <p class="border-1 rounded-full border-red-700 grid place-items-center bg-red-500 text-white  mb-4 mt-2">{{$message}}</p>
        @enderror
        <button type="submit" class="block mx-auto w-2/3 bg-green-700 mt-4 py-2 rounded-2xl text-white font-semibold mb-3">Añadir</button>
        </form>
    </div>
